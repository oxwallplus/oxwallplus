...
if(defined('OW_URL_HOME')) {
    try {
        $dbReady = (bool) OW::getConfig()->getValue('base', 'site_installed');
        $complete = (bool) OW::getConfig()->getValue('base', 'install_complete');
    } catch(Exception $e) {
        $dbReady = false;
	$complete = false;
    }
...
if(!$complete || (defined('OW_INSTALL_DEV') && OW_INSTALL_DEV)) {
    if(!defined('OW_URL_HOME')) {
        $url = UTIL_Url::selfUrl();

        if(substr($url, -1) != '/') {
            $url .= '/';
        }

        $installUrl = strpos($url, '/install');        

        if($installUrl) {
            $url = substr($url, 0, $installUrl) . '/';
        }
        
        $installDir = strpos($url, '/install');        
        if($installDir) {
            $url = substr($url, 0, $installDir) . '/';
        }

        define('OW_URL_HOME', $url);
    }

    define('INSTALL_URL_ROOT', OW_URL_HOME . 'install/');
    define('INSTALL_URL_VIEW', OW_URL_HOME . 'install/view/');

    define('INSTALL_DIR_ROOT', dirname(__FILE__) . DS);
    define('INSTALL_DIR_LANG', INSTALL_DIR_ROOT . 'langs' . DS);
