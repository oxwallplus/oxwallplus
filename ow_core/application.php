...
class OW_Application
{
    ...
    public function init() {
        PERFORMANCE_BOL_Service::getInstance()->setCache();
        
        // router init - need to set current page uri and base url
        $baseUrl = $this->detectLanguage();
        $router = OW::getRouter();
        ...
        if(empty($timeZone)) {
            date_default_timezone_set(OW::getConfig()->getValue(LOCALIZATION_BOL_Service::KEY, 'siteTimezone'));
        }
        ...
        $viewRenderer->assignVar('oxwallplus',
            '<a href="https://oxwallplus.com/" target="_blank" title="Powered by OxwallPlus">OxwallPlus</a>');
    ...
    public function handleRequest() {
        //members only
        if((int) OW::getConfig()->getValue('base', 'guests_can_view') === BOL_UserService::PERMISSIONS_GUESTS_CANT_VIEW && !OW::getUser()->isAuthenticated()) {
            $attributes = array(
        ...
        // password protected
        if((int) OW::getConfig()->getValue('base', 'guests_can_view') === BOL_UserService::PERMISSIONS_GUESTS_PASSWORD_VIEW && !OW::getUser()->isAuthenticated() && !isset($_COOKIE['base_password_protection'])) {
            $attributes = array(
                OW_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_CTRL_BaseDocument',
        ...
        // maintenance mode
        if((bool) OW::getConfig()->getValue('base', 'maintenance') && !OW::getUser()->isAdmin()) {
            $attributes = array(
                OW_RequestHandler::CATCH_ALL_REQUEST_KEY_CTRL => 'BASE_CTRL_BaseDocument',
    ...
    protected function newDocument() {
        ...
        $document->addScript(OW::getPluginManager()->getPlugin('base')->getStaticJsUrl() . 'jquery/jquery.min.js',
            'text/javascript', (-100));
    ...
    protected function httpVsHttpsRedirect() {
        ...
        foreach($this->httpsHandlerAttrsList as $item) {
            if($item[OW_RequestHandler::ATTRS_KEY_CTRL] == $attrs[OW_RequestHandler::ATTRS_KEY_CTRL] && (empty($item[OW_RequestHandler::ATTRS_KEY_ACTION]) 
                    || $item[OW_RequestHandler::ATTRS_KEY_ACTION] == $attrs[OW_RequestHandler::ATTRS_KEY_ACTION])) {
                $specAttrs = true;
                if(!$isSsl) {
    ...
    protected function detectLanguage() {
        $config = OW::getConfig();
        $displayUrl = (int)$config->getValue(LOCALIZATION_BOL_Service::KEY, 'urlLanguage');
        if($displayUrl != 2) {
            $this->urlHostRedirect();
        }
        $service = LOCALIZATION_BOL_LanguageService::getInstance();
        $baseUrl = OW_URL_HOME;
        $language = $service->findDefault();
        ...
        if($displayUrl) {
            if($displayUrl == 1) {
                ...
                $isoCode = $uri[1];
                if(strlen($isoCode) == 2) {
                   if($language = $service->findByIsoCode($isoCode)) {
                       $baseUrl .= $isoCode . DS;
                   }
                }
            } elseif($displayUrl == 2) {
                ...
                if(strlen($isoCode) == 2) {
                    if($language = $service->findByIsoCode($isoCode)) {
                        $baseUrl = (($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http") . '://' . $isoCode . '.' . str_replace($isoCode.'.', '', $host) . DS;
                    }
                }
            }
            $service->setCurrentLanguage($language);
        } else {
            $languageId = (int)OW::getRequest()->getParam('language_id');
            if($languageId) {
                $language = $service->findById($languageId);
                if($language !== null && $language->getStatus() == "active") {
                    $service->setCurrentLanguage($language);
                }
            }
        }
