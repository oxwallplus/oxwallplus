...
class Form
{
    ...
    public function addElement($element) {
        if(!$element instanceof FormElement && !$element instanceof FORM_Element) {
        ...
        if($element instanceof Submit || $element instanceof ELEMENT_Submit) {
    ...
    public function isValid($data) {
        ...
        foreach($this->elements as $element) {
            if($element instanceof ELEMENT_File) {
                ...
                $element->setValue($file['name']);
                $element->setSize($file['size']);
                $element->setFile($file['tmp_name']);
                $this->files[] = $element;
            } else {
    ...
    public function uploadFiles() {
        ...
        foreach($this->files as $file) {
            if(is_uploaded_file($file->getFile()) && $file->getUploadPath()) {
                $storage = OW::getStorage();
                $filename = UTIL_File::sanitizeName($file->getValue());
                if($storage->copyFile($file->getFile(), $file->getUploadPath() . $filename)) {
                    $result[] = array(
                        'filename' => $filename,
                        'path' => $file->getUploadPath()
                    );
                    OW::getFeedback()->info(OW::getLanguage()->text("base", "form_file_upload_success"));
                } else {
                    OW::getFeedback()->error(OW::getLanguage()->text("base", "form_file_upload_error"));
                }
            } else {
                OW::getFeedback()->error(OW::getLanguage()->text("base", "form_file_upload_error"));
            }
    ...
    public function render($formContent, array $params = array()) {
        ...
        foreach($this->elements as $value) {
            if($value instanceof HiddenField || $value instanceof ELEMENT_Hidden) {
