...
class OW_RequestHandler
{
    ...
    protected function getControllerActionDefaultTemplate(OW_ActionController $ctrl) {
        ...
        $filename = OW::getAutoloader()->classToFilename($this->handlerAttributes[self::ATTRS_KEY_CTRL], false) . '_'
             . OW::getAutoloader()->classToFilename(ucfirst($this->handlerAttributes[self::ATTRS_KEY_ACTION]), false) . '.html';

        $mobile = $ctrl instanceof OW_MobileActionController ? true : false;
        if(!$template = OW::getThemeManager()->findTemplate($filename, $plugin, 'controllers', $mobile)) {
            $template = ($mobile ? $plugin->getMobileCtrlViewDir() : $plugin->getCtrlViewDir()) . $filename;
        }
        return $template;
    }
