...
//        }
    ...
    public function findTemplate($name, $plugin, $type = null, $mobile = false) {
        $themeDir = $this->getCurrentTheme()->getRootDir($mobile);
        $template = $themeDir . ($plugin->isSystem() ? OW_DIR_THEME_SYSTEM_PLUGIN : OW_DIR_THEME_PLUGIN) . $plugin->getKey() . DS . ($type ? $type . DS : '') . $name;
