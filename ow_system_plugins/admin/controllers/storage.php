...
class ADMIN_CTRL_Storage extends ADMIN_CTRL_StorageAbstract
{
    ...
    public function platformUpdateRequest() {
        ...
        $info = $this->storageService->getPlatformInfoForUpdate();

        if(!$info) {
            return;
        }

        $newVersion = (int) substr($info['build'], -6, 1).'.'.(int)substr($info['build'], -4, 2).'.'.(int) substr($info['build'], -2, 1);
        $params = array(
        ...
        if ( isset($info["minPhpVersion"]) && version_compare(PHP_VERSION, trim($newPlatformInfo["minPhpVersion"])) < 0 )
        {
