...
class JoinForm extends BASE_CLASS_UserQuestionForm
{
    ...
    const SESSION_QUESTION_LIST = 'join.question_list';
    ...
    public function __construct( $controller )
    {
        ...
        if ( OW::getRequest()->isPost() )
        {
            ...
            if ( empty($this->sortedQuestionsList) )
            {
                $this->sortedQuestionsList = array();
            }

            if ( $this->sortedQuestionsList )
            {
                foreach ( $this->sortedQuestionsList as $key => $value )
                {
                    $newKey = array_search($value, $this->sortedQuestionsList);

                    if ( $newKey !== false && isset($_POST[$key]) && isset($this->sortedQuestionsList[$newKey]) )
                    {
        ...
        OW::getSession()->set(self::SESSION_QUESTION_LIST, $this->sortedQuestionsList);
    ...
    protected function init( array $accounts )
    {
        ...
        {
            $joinAccountType = new ELEMENT_Select('accountType');
            $joinAccountType->setLabel(OW::getLanguage()->text('base', 'questions_question_account_type_label'), array('class' => 'col-form-label'));
        ...
        public function checkSession()
    {
        ...
        if ( !empty($sessionStamp) && $stamp > $sessionStamp )
        {
            ...
            OW::getSession()->delete(self::SESSION_QUESTION_LIST);
    ...
    public function setQuestionsLabel()
    {
        foreach ( $this->sortedQuestionsList as $question )
        {
                ...
                if( !empty($data['label']) )
                {
                    $this->getElement($question['name'])->setLabel($data['label'], array('class' => 'col-form-label'));
                }
                else
                {
                    $this->getElement($question['name'])->setLabel(OW::getLanguage()->text('base', 'questions_question_' . $question['name'] . '_label'), array('class' => 'col-form-label'));
                }
    ...
    protected function updateJoinData()
    {
        ...
        $list = OW::getSession()->get(self::SESSION_QUESTION_LIST);
        if ( !empty($list) )
        {
            foreach ( $list as $name => $value )
            {
                if ( isset($joinData[$name]) )
                {
                    unset($this->data[$name]);
                    $this->data[$name] = $joinData[$name];
                }
            }
        }
    ...
    public function getRealValues()
    {
        ...
        unset($vaues['form_name']);
        unset($vaues['csrf_token']);
        return $values;
    }
    ...
    protected function addLastStepQuestions( $controller )
    {
        ...
        switch ( $displayPhotoUpload )
        {
            ...
            case BOL_UserService::CONFIG_JOIN_DISPLAY_PHOTO_UPLOAD :
                ...
                $userPhoto->setLabel(OW::getLanguage()->text('base', 'questions_question_user_photo_label'), array('class' => 'col-form-label'));
        ...
        $event = new OW_Event(SECURITY_BOL_Service::EVENT_GET_CAPTCHA);
        OW::getEventManager()->trigger($event);
        $captcha = $event->getData();
        
        if($captcha && $captcha instanceof ELEMENT_Captcha) {
            $this->addElement($captcha);
        }

        $controller->assign('captcha', $captcha);
    ...
    protected function addFieldValidator( $formField, $question )
    {
        ...
        if ( (string) $question['base'] === '1' )
        {
            ...
            if ( $question['name'] === 'password' )
            {
                ...
                $passwordRepeat->setLabel(OW::getLanguage()->text('base', 'questions_question_repeat_password_label'), array('class' => 'col-form-label'));
    ...
    {
        $realQuestionList = OW::getSession()->get(self::SESSION_QUESTION_LIST);
