...
class BOL_PluginService
{
    ...
    public function findSystemPlugins() {
     ...
     * @return array|null
     */
    public function readPluginXmlInfo($xml) {
        if(!file_exists($xml)) {
            return null;
        }
        ...
        $properties = array("key", "name", "description", "license", "author", "build", "copyright", "licenseUrl");
     ...
     * @return array
     */
    public function findRegularPlugins($active = false)
    {
        ...
        {
            if ( !$plugin->isSystem() && (!$active || ($active && $plugin->isActive())))
            {
...
// install plugins list
    ...
    private function updatePluginListCache($clear = true) {
        if($clear) {
            $this->pluginDao->clearCache();
        }
    ...
    private function getPluginListCache() {
        if(!$this->pluginListCache) {
            $this->updatePluginListCache(false);
        }
