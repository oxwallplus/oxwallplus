...
class BOL_MenuItemDao extends OW_BaseDao
{
    ...
    public function findMenuItems( $menuType )
    {
        return $this->dbo->queryForList("
			...
			WHERE `mi`.`" . self::TYPE . "` = :menuType ORDER BY `mi`.`order` ASC", array('menuType' => $menuType), self::CACHE_TIME, array(self::CACHE_TAG_MENU_TYPE_LIST, OW_CacheManager::TAG_OPTION_INSTANT_LOAD));
    }
    ...
    public function findMenuItemsForMenuTypes( $menuTypes )
    {
        return $this->dbo->queryForList("
			...
			FROM `" . $this->getTableName() . "` AS `mi`
			LEFT JOIN `" . $this->documentDao->getTableName() . "` AS `d` ON ( `mi`.`" . self::DOCUMENT_KEY . "` = `d`.`" . BOL_DocumentDao::KEY . "`)
			WHERE `mi`.`" . self::TYPE . "` IN (" . $this->dbo->mergeInClause($menuTypes) . ") ORDER BY `mi`.`order` ASC", array(), self::CACHE_TIME, array(self::CACHE_TAG_MENU_TYPE_LIST));
    }
    ...
    public function findFirstLocal($visibleFor, $menuType) {
        ...
        return $this->dbo->queryForObject("
			...
			ORDER BY `order` ASC
			LIMIT 1", $this->getDtoClassName(), array($visibleFor, $menuType), self::CACHE_TIME, array(self::CACHE_TAG_MENU_TYPE_LIST));
    }
    ...
    public function clearCache() {
        OW::getCacheManager()->clean(array(self::CACHE_TAG_MENU_TYPE_LIST));
    }
}
