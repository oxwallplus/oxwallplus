...
{
    const UPDATE_OXWALL_PLUS_SERVER = "https://oxwallplus.com/api/downloads/";
    const UPDATE_SERVER = "https://storage.oxwall.org/";
    const INFO_XML = 'info.xml';
    const TYPE_CORE = 'core';
    const TYPE_PLUGIN = 'plugin';
    const TYPE_THEME = 'theme';
    /* ---------------------------------------------------------------------- */
    const URI_CHECK_ITEMS = "updates";
    ...
    public function checkUpdates() {
        ...
        $request = array();
        $request[] = array(
            self::URI_VAR_ITEM_TYPE => self::TYPE_CORE,
            self::URI_VAR_BUILD => OW::getConfig()->getValue("base", "soft_build"),
            self::URI_VAR_KEY => self::URI_VAR_PLATFORM
        );
        
        $data = $this->triggerEventBeforeRequest();
        $data["items"] = json_encode($request);

        $params = new UTIL_HttpClientParams();
        $params->addParams($data);
        $response = UTIL_HttpClient::post($this->getOxwallPlusStorageUrl(self::URI_CHECK_ITEMS), $params);
        ...
        if(!empty($result["data"]["updates"])) {
            $updates = $result["data"]["updates"];
            if(isset($updates[self::URI_VAR_PLATFORM])) {
                OW::getConfig()->saveConfig("base", "update_soft", 1);
            }
        }
                
        $requestArray = array("items" => array());
    ...
    public function getPlatformInfoForUpdate() {
        $data[self::URI_VAR_ITEM_TYPE] = self::TYPE_CORE;
        $data[self::URI_VAR_KEY] = self::URI_VAR_PLATFORM;
        $url = OW::getRequest()->buildUrlQueryString(self::UPDATE_OXWALL_PLUS_SERVER, $data);
    ...
    public function downloadPlatform() {
        ...
        $data = array_merge($params, $this->triggerEventBeforeRequest($params));
        $requestUrl = OW::getRequest()->buildUrlQueryString($this->getStorageUrl(self::URI_DOWNLOAD),
            $data);

        $paramsObj = new UTIL_HttpClientParams();
        $paramsObj->addParams($data);
        $response = UTIL_HttpClient::get($this->getOxwallPlusStorageUrl(self::URI_DOWNLOAD), $paramsObj);
    ...
    public function getXmlInfo($xml) {
        ...
        $info = (array)simplexml_load_file($xml);
        $properties = array("type", "key", "name", "description", "license", "author", "build", "copyright", "licenseUrl");
    ...
    public function getPlatformXmlInfo() {
        $filePath = OW_DIR_ROOT . self::INFO_XML;
    ...
    private function getOxwallPlusStorageUrl($uri) {
        return self::UPDATE_OXWALL_PLUS_SERVER . UTIL_String::removeFirstAndLastSlashes($uri) . "/";
    }
