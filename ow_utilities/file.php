...
class UTIL_File
{
     ...
     * @return bool
     */
    public static function validate($file, array $extensions = array()) {
        $file = trim($file);
        if(!$file) {
            return false;
        }
        ...
        if(empty($extensions)) {
            $extensions = array_merge(self::$imageExtensions, self::$videoExtensions);
        }
    ...
    public static function sanitizeName($file) {
        ...
        $specialChars = array("?", "[", "]", "/", "\\", "=", "<", ">", ":", ";", ",", "'", "\"", "&", "$", "#", "*", "(",
            ")", "|", "~", "`", "!", "{", "}");
        $file = str_replace($specialChars, '', $file);
