...
var OwUtils = function(){
    ...
    this.slug = function(original, slug, url) {
        $('#' + original).bind('change', function(e) {
            if($(this).val().length > 1) {
                $.ajax({
                    type: 'get',
                    url: url,
                    data: { 'title' : $('#' + original).val() },
                    dataType: 'json',
...
var OwAvatarField = function( id, name, params ){
    ...
    $preview.find("span").click(function(e){
        ...
        $preview.hide();
        $(formElement.input).val("").parent().show();
